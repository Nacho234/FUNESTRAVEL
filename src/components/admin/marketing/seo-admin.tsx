        ...
        <SectionCard title="Metadata por página" description="Título, description e imagen social de las páginas principales.">
          <label className="block text-sm">
            ...
            <select
              ...
              className="w-full rounded-[var(--radius-control)] border border-black/[0.07] px-3 py-2 font-mono text-sm cursor-pointer focus:border-teal-500 focus:outline-none"
            >
              {seoPages.map((p) => (
          ...
          <label className="mt-3 block text-sm">
            ...
            <input value={meta.title} onChange={(e) => setMeta((m) => ({ ...m, title: e.target.value }))} className="w-full rounded-[var(--radius-control)] border border-black/[0.07] px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" />
          ...
          <label className="block text-sm">
            ...
            <textarea rows={3} value={meta.description} onChange={(e) => setMeta((m) => ({ ...m, description: e.target.value }))} className="w-full rounded-[var(--radius-control)] border border-black/[0.07] px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" />
          ...
          <label className="block text-sm">
            ...
            <input value={meta.ogImage} onChange={(e) => setMeta((m) => ({ ...m, ogImage: e.target.value }))} placeholder="URL 1200x630" className="w-full rounded-[var(--radius-control)] border border-black/[0.07] px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" />
          </label>
          ...
          <SectionCard title="Redirecciones" description="301 permanentes para URLs viejas o contenido vencido.">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-2">
              <label className="block text-xs">
                ...
                <input value={newFrom} onChange={(e) => setNewFrom(e.target.value)} placeholder="/ruta-vieja" className="w-36 rounded-[var(--radius-control)] border border-black/[0.07] px-2.5 py-1.5 font-mono text-xs focus:border-teal-500 focus:outline-none" />
              </label>
              <label className="block text-xs">
                <span className="mb-1 block font-semibold text-graphite-700">Destino</span>
                <input value={newTo} onChange={(e) => setNewTo(e.target.value)} placeholder="/ruta-nueva" className="w-36 rounded-[var(--radius-control)] border border-black/[0.07] px-2.5 py-1.5 font-mono text-xs focus:border-teal-500 focus:outline-none" />
              </label>
              <AdminButton
                size="sm"
                variant="secondary"
